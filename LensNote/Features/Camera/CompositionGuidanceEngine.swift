...
final class CompositionGuidanceEngine {
    ...
    private func snapshot(from features: FrameFeatures, evaluation: CompositionEvaluation) -> CompositionFeatureSnapshot {
        ...
        CompositionFeatureSnapshot(
            ...
            detectedRectX: evaluation.detectedSubjectRect.map { Double($0.origin.x) },
            detectedRectY: evaluation.detectedSubjectRect.map { Double($0.origin.y) },
            detectedRectWidth: evaluation.detectedSubjectRect.map { Double($0.width) },
            detectedRectHeight: evaluation.detectedSubjectRect.map { Double($0.height) },
